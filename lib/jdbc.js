...
class JDBC {
  ...
  connect(callback) {
    ...
    if (this.connected) {
      console.info('Already connected to AS400. Skipping connect call.');
      return callback();
    }
  ...
  close(callback) {
    ...
    if (!this.connected) {
      console.info('Not connected to the AS400. Skipping close.');
      return callback();
    }
  ...
  /**
   * Executes a prepared statement and return results.
   * Optimized for large datasets with batch processing support.
   * @param sql - The sql string.
   * @param parameters - Parameters array.
   * @param fetchSize - Number of rows to fetch at a time (default: 1000 for migrations).
   * @param callback - The finished callback function.
   */
  query(sql, parameters, fetchSize, callback) {
    // variables that need access in finally block.
    let connection = null;
    let statement = null;
    let error = null;
    ...
    try {
      ...
      // Set optimal fetch size for large data migration
      // Default to 1000 if not specified for better memory management
      const optimalFetchSize = fetchSize !== undefined && fetchSize !== null ? fetchSize : 1000;
      statement.setFetchSizeSync(optimalFetchSize);
      ...
      // get the results with optimized processing.
      results = getResultsOptimized(rs);
    }
    catch(ex) {
      this.logger.error(`Error in query: ${ex.message}`);
      error = ex;
    }
    finally {
      try {
  ...
  runTransaction(executeFunction, callback) {
    ...
    try {
      ...
      this.logger.error(`Error in runTransaction: ${ex.message}`);
      return callback(ex)
    }
...
/**
 * Converts a result set into an array with optimized memory handling.
 * Pre-allocates column name array to avoid repeated metadata lookups.
 * NOTE: This throws an exception. Needs to be handled in the calling function.
 * @param rs - The result set object.
 */
function getResultsOptimized(rs) {
  // store the results.
  let results = [];
  let currentIndex = 0;
  ...
  // Pre-fetch column names to avoid repeated metadata lookups
  let columnNames = [];
  for (let i = 1; i <= cc; i++) {
    columnNames.push(rsmd.getColumnNameSync(i));
  }
  ...
  while (next) {
    ...
    // loop for each column using pre-fetched names.
    for (let i = 0; i < cc; i++) {
      row[columnNames[i]] = Utils.trimValue(rs.getStringSync(i + 1));
    }
